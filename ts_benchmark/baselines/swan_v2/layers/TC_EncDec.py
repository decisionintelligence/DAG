...
from ts_benchmark.baselines.swan_v2.layers.Embed import PatchEmbedding, CompressAndProject
from ts_benchmark.baselines.swan_v2.layers.SelfAttention_Family import FullAttention, AttentionLayer
from ts_benchmark.baselines.swan_v2.layers.Transformer_EncDec import Encoder, EncoderLayer
...
class ExogenousWeightModule(nn.Module):
    def __init__(
        ...
        self.weight_gate_sharpness = weight_gate_sharpness
    ...
    def forward(self, patch_exog, batch_size, exog_vars):
        ...
        self._update_tau()
        tau = self.tau.detach()
        # SWAN v2: 单阈值平滑门控，避免双阈值导致的过早硬裁剪
        gate_scale = self.weight_gate_sharpness / (tau + 1e-6)
        soft_weight = torch.sigmoid((raw_weight - self.threshold) * gate_scale)
        ...
        # SWAN v2: 在门控输出上施加L1，避免直接压扁中间表示
        sparse_loss = self.lambda3 * ramp * torch.mean(torch.abs(soft_weight))
    ...
    def __init__(
        ...
        if self.exog_dim > 0:
            self.exog_weight = ExogenousWeightModule(
                ...
                weight_gate_sharpness=weight_gate_sharpness,
            )
        else:
