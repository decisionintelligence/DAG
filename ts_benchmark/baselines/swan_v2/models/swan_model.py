...
from ts_benchmark.baselines.swan_v2.layers.CC_EncDec import CovCausalityEncoder
from ts_benchmark.baselines.swan_v2.layers.TC_EncDec import TemporalCausalityEncoder
...
class SWANV2Model(nn.Module):
    def __init__(self, config):
        super(SWANV2Model, self).__init__()
    ...
    def forward(self, input, exog_future):
        ...
        sparse_loss = weight_sparse_loss + self.mask_sparsity_lambda * mask_sparse_loss
        if self.dynamic_scheduler is not None:
            sparse_beta = self.dynamic_scheduler(causality_loss.detach())
