...
from ts_benchmark.baselines.swan_v3.layers.Embed import PatchEmbedding, CompressAndProject
...
class ExogenousWeightModule(nn.Module):
    def __init__(
        ...
        self.weight_gate_floor = weight_gate_floor
    ...
    def forward(self, patch_exog, batch_size, exog_vars):
        ...
        gated = torch.sigmoid((raw_weight - self.threshold) * gate_scale)
        soft_weight = self.weight_gate_floor + (1 - self.weight_gate_floor) * gated
    ...
    def __init__(
        ...
        if self.exog_dim > 0:
            self.exog_weight = ExogenousWeightModule(
                ...
                weight_gate_floor=weight_gate_floor,
            )
        else:
